...
@dataclass(frozen=True)
class ArbParams:
    entry_rate_diff_pct: float = field(default_factory=lambda: _env_float("ENTRY_FUNDING_RATE_DIFF_PCT", 25.0))
    exit_rate_diff_pct: float = field(default_factory=lambda: _env_float("EXIT_FUNDING_RATE_DIFF_PCT", 5.0))
    ...
    min_profit_threshold_usd: float = field(default_factory=lambda: _env_float("MIN_PROFIT_THRESHOLD_USD", 5.0))
